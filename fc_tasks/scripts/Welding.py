...
class Welder():
    ...
    def laser_ready_arm(self):
        ...
        if rpc.iovalrd(self.server, rpc.IoType.DigitalOut, 43) == 0:
        ...
        while time.time() - start_time < self.timeout: 
            if rpc.iovalrd(self.server, rpc.IoType.DigitalOut, index=47) == 1: # READY FOR EXTERNAL CON
                break
            time.sleep(self.poll_interval)
        else:
            self.laser_error("LASER RDY TIMEOUT")
            return
        ...
        start_time = time.time()
        while time.time() - start_time < self.timeout: 
            if rpc.iovalrd(self.server, rpc.IoType.DigitalOut, index=45) == 1: # LASER IN STANDBY STATE
                break
            time.sleep(self.poll_interval)
        else:
            self.laser_error("LASER RDY TIMEOUT")
        ...
        start_time = time.time()
        while time.time() - start_time < self.timeout: 
            if rpc.iovalrd(self.server, rpc.IoType.DigitalOut, index=43) == 1: # LASER IN EMISSION STAT
                break
            time.sleep(self.poll_interval)
        else:
            self.laser_error("LASER RDY TIMEOUT")
    ...
    def weld_start(self): 
        # Also starts gas
        if rpc.iovalrd(self.server, rpc.IoType.DigitalIn, index=27) == 1:
            print("Gas Fault Detected")
            return
        elif rpc.iovalrd(self.server, rpc.IoType.DigitalIn, index=28) == 1:
            ...
            start_time = time.time()
            while time.time() - start_time < self.timeout: 
                if rpc.iovalrd(self.server, rpc.IoType.DigitalOut, index=25) == 1: #ARC DETECT
                    break
                time.sleep(self.poll_interval)
            else:
