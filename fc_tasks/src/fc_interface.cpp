...
void FC_Interface::jointStatesCallback(const sensor_msgs::JointState::ConstPtr &msg)
{
	...
	// ros::AsyncSpinner async_spinner(1);                                                                                                                        async_spinner.start();

	tool0_pose_pub_.publish(move_group_.getCurrentPose());

	// async_spinner.stop();

	//        geometry_msgs::PoseStamped pose;
	//        tool0_pose_pub_.publish(pose);
}
												...
												double blend_radius, bool async, std::string planner_id)
{
	...
	for (int i = 0; i < waypoints.size(); i++)
	{
		...
		waypoint_pose.header.frame_id = "/real/base_link";
